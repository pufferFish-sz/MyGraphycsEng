	...
	for (uint32_t s = 0; s < samples.size(); ++s) {
		...
		if constexpr (primitive_type == PrimitiveType::Lines) {
			...
			for (uint32_t i = 0; i + 2 < clipped_vertices.size(); i += 3) {
				std::vector<ClippedVertex> clipped_vertices_copy = clipped_vertices;
				clipped_vertices_copy[i].fb_position += sample_offset;
				clipped_vertices_copy[i + 1].fb_position += sample_offset;
				clipped_vertices_copy[i + 2].fb_position += sample_offset;
				rasterize_triangle(clipped_vertices_copy[i], clipped_vertices_copy[i + 1], clipped_vertices_copy[i + 2], emit_fragment);
			}
		}
		else {
			static_assert(primitive_type == PrimitiveType::Lines, "Unsupported primitive type.");
